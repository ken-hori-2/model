...
def main():
    ...
    for i in range (10):
        ...
        if prev_action == X[0]: # UP
            ...
            E_0[1].append(random.choice(A_0))                           # At-1 = UPの時の At(← →)
            E_0[2].append(R[0])                                         # 今はUP以外は報酬0
            "test 0823"
            # E_0[2].append(R[1])
        ...
        if prev_action == X[2]: # LEFT
            ...
            E_2[1].append(random.choice(A_2))                           # At-1 = LEFTの時の At(↑ ↓)
            if E_2[1][-1] == A_2[0]: # UPならRt = 1
                print("⚡️ LEFT -> UP Rt = 1")
                E_2[2].append(R[1]) # 今は次の行動で必ず発見できる前提(R = 1)
                # E_2[2].append(random.choice(R))
            else:
                E_2[2].append(R[0])
        if prev_action == X[3]: # RIGHT
            E_3[0].append(prev_action)
            E_3[1].append(random.choice(A_3))                           # At-1 = RIGHTの時の At(↑ ↓)
            if E_3[1][-1] == A_3[0]: # UPならRt = 1
                print("⚡️ RIGHT -> UP Rt = 1")
                E_3[2].append(R[1]) # 今は次の行動で必ず発見できる前提(R = 1)
